...
struct DCEL{T}
    ...
    halfedges::Vector{MVector{6,Int}}
...
Base.getindex(h::HalfedgeHandle) = h.dcel.halfedges[h.i]
...
nhalfedges(dcel::DCEL) = length(dcel.halfedges)
halfedges(dcel::DCEL) = map(i -> dcel[Halfedge, i], eachindex(dcel.halfedges))
...
function DCEL(segments::AbstractVector{DirSegment{2, T}}) where {T}
    ...
    halfedges = [
        let i = (I+1) ÷ 2, main = isodd(I)
            MVector(
                main ? i                : -i            , # 1 self
                main ? i                : mod(i+1, 1:n) , # 2 vertex
                main ? 1                : 0             , # 3 face
                main ? 2mod(i-1, 1:n)-1 : 2mod(i+1, 1:n), # 4 prev
                main ? 2mod(i+1, 1:n)-1 : 2mod(i-1, 1:n), # 5 next
                main ? 2i               : 2i-1          , # 6 twin, if isodd(i) 2i-1 elseif iseven(i) 2i end
            )
        end for I in 1:2n
    ]
...
halfedges(dcel::DCEL) = Base.Iterators.map(i -> dcel[Halfedge, i], eachindex(dcel.halfedges))
...
function insertdiagonal!(dcel::DCEL{T}, v1::VertexHandle, v2::VertexHandle, f::FaceHandle{T}) where {T}
    ...
    insert_diagonal!(e1, e2)
end

insertdiagonal!(dcel::DCEL{T}, e1::Integer, e2::Integer) where {T} = insert_diagonal!(dcel[Halfedge, e1], dcel[Halfedge, e2])
...
function explode(dcel::DCEL{T}) where {T}
    nf = length(Iterators.faces(dcel))
    polys = DCEL{T}[]
    for f in Iterators.faces(dcel)
        segments = collect(first(Iterators.segments(f)))
        if nf != 1
            push!(polys, DCEL(segments))
        end
        nf -= 1
    end
    return polys
